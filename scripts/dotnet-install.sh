...
# Downloaded from https://learn.microsoft.com/en-us/dotnet/core/install/linux-scripted-manual#scripted-install
...
get_remote_file_size() {
    ...
    # Modified original script to only print if num > 0

    if machine_has "curl"; then
        file_size=$(curl -sI  "$zip_uri" | grep -i content-length | awk '{ num = $2 + 0; if (num > 0) print num }')
    elif machine_has "wget"; then
        file_size=$(wget --spider --server-response -O /dev/null "$zip_uri" 2>&1 | grep -i 'Content-Length:' | awk '{ num = $2 + 0; if (num > 0) print num }')
    else
