...
namespace Stripe.Tests
{
	public class when_listing_charges_with_paging
	{
		...
		Establish context = () =>
		{
			// Because we want to page through a known set of data, we're creating a client for this test.
			...
			_chargeIds.Add(_stripeChargeService.Create(chargeCreateOptions).Id);
			_chargeIds.Add(_stripeChargeService.Create(chargeCreateOptions).Id);
			_chargeIds.Reverse();			// so it matches up with the order we should get back from Stripe
		};
