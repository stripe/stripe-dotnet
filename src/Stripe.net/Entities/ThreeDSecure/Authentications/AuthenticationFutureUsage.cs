// File generated from our OpenAPI spec
namespace Stripe.ThreeDSecure
{
    using Newtonsoft.Json;
    using Stripe.Infrastructure;
    using STJS = System.Text.Json.Serialization;

    [STJS.JsonConverter(typeof(STJStripeEntityConverter))]
    public class AuthenticationFutureUsage : StripeEntity<AuthenticationFutureUsage>
    {
        /// <summary>
        /// Details about installment payments.
        /// </summary>
        [JsonProperty("installment")]
        [STJS.JsonPropertyName("installment")]
        public AuthenticationFutureUsageInstallment Installment { get; set; }

        /// <summary>
        /// Details about recurring payments.
        /// </summary>
        [JsonProperty("recurring")]
        [STJS.JsonPropertyName("recurring")]
        public AuthenticationFutureUsageRecurring Recurring { get; set; }

        /// <summary>
        /// The type of future usage declared for this 3DS Authentication.
        /// One of: <c>card_on_file</c>, <c>installment</c>, or <c>recurring</c>.
        ///
        /// This enum can grow over time; additional values may be added in the future.
        /// </summary>
        [JsonProperty("type")]
        [STJS.JsonPropertyName("type")]
        public string Type { get; set; }
    }
}
