...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            long tolerance = EventUtility.DefaultTimeTolerance)
        {
            ...
            // since the base EventNotification class does not expose RelatedObject.
            var jsonObj = Newtonsoft.Json.Linq.JObject.Parse(json);
            var typeToken = jsonObj["type"];
            if (typeToken != null && typeToken.ToString().StartsWith("v2.extend.objects.object_record"))
            {
                var relatedObj = jsonObj["related_object"] as Newtonsoft.Json.Linq.JObject;
                var roType = relatedObj?["type"]?.ToString();
                if (roType != null)
                {
                    jsonObj["type"] = typeToken.ToString().Replace("v2.extend.objects.object_record", roType);
