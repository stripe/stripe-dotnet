// File generated from our OpenAPI spec
namespace Stripe.V2.Signals
{
    using Newtonsoft.Json;
    using Stripe.Infrastructure;
    using STJS = System.Text.Json.Serialization;

    [STJS.JsonConverter(typeof(STJStripeOptionsConverter))]
    public class AccountEvaluationCreateAccountActivityDetailsOptions : INestedOptions
    {
        /// <summary>
        /// The ID of an existing account activity to associate with the evaluation.
        /// </summary>
        [JsonProperty("account_activity")]
        [STJS.JsonPropertyName("account_activity")]
        public string AccountActivity { get; set; }

        /// <summary>
        /// Inline activity data used to create a new account activity for the evaluation.
        /// </summary>
        [JsonProperty("data")]
        [STJS.JsonPropertyName("data")]
        public AccountEvaluationCreateAccountActivityDetailsDataOptions Data { get; set; }
    }
}
